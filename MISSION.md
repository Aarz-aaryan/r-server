...
## Service Status (2026-06-15)

| | Service | Container | Port | Status | Notes |
|-|---------|-----------|------|--------|-------|
| ✅ | Jellyfin | jellyfin | 8096 | UP 4 days | Movies + TV |
| ✅ | Immich | immich | 2283 | UP 39h | **Data on HDD** — library (15GB), backups (33MB) |
...
| ✅ | Uptime Kuma | uptime-kuma | 3001 | UP 39h | Monitors all services |
| ✅ | Jackett | jackett | 9117 | UP 39h | Torznab, FlareSolverr configured |
| ✅ | Prowlarr | prowlarr | 9696 | UP 39h | Indexer management |
| ✅ | qBittorrent | qbittorrent | 6811 | UP 39h | Torrent downloads |
| ✅ | Homepage | homepage | 8383 | UP 39h | **Replaced Dashy** — r-server dashboard |
| ✅ | FlareSolverr | flaresolverr | 8191 | UP 39h | Cloudflare bypass |
| ✅ | Glances | glances | 61208 | UP 5 days | System monitoring agent |
| ✅ | Vaultwarden | vaultwarden | 8443 | UP 39h | Password manager — **HTTPS only** |
| ✅ | slskd | slskd | 5030-5031 | UP 39h | Soulseek client |
| ✅ | Reclaimerr | reclaimerr | 8977 | UP 43h | Media library cleanup |
| ✅ | docker-redis-1 | docker-redis-1 | 6379 | UP 39h | Redis for jackett/prowlarr |
| ✅ | r-server-redis-1 | r-server-redis-1 | 6379 | UP 39h | Redis |
...
## Port Access Map
```
80    → (redirect to Vaultwarden HTTPS)
443   → (redirect to Vaultwarden HTTPS)
8443  → Vaultwarden (password manager, HTTPS only)
8096  → Jellyfin (movies + TV)
2283  → Immich (photo/video backup)
8383  → Homepage (r-server dashboard, replaced Dashy)
...
├── encoded-video/   ← Transcoded videos (on SSD — symlinked from library)
├── thumbs/          ← Thumbnails
├── backups/         ← Immich DB backups (33MB)
├── profile/         ← User profiles
├── upload/          ← Temp upload staging
...
| Vaultwarden | https://100.84.224.18:8443 | aarz@aarz.com | aarz1947 |
...
- **Vaultwarden HTTPS only:** HTTP on port 80 redirects to HTTPS on 8443. The `ADMIN_TOKEN` is set but admin page access requires the token via `?admin_token=` query param.
- **Nginx → Vaultwarden proxy:** Uses vaultwarden container IP `172.19.0.5`. If vaultwarden restarts and gets a new IP, update `/etc/nginx/sites-enabled/vaultwarden` and `sudo nginx -s reload`.
- **Stats server (port 9400):** Requires bearer auth — no token configured (401 if no auth header sent).
...
- [x] **Dashy removed, replaced with Homepage** — RESOLVED (2026-06-15).
- [x] **Vaultwarden HTTPS 502** — RESOLVED. Nginx proxy IP updated from `172.19.0.2` → `172.19.0.5`.
